...
🏗 Архитектура
    ...
    sysmon-agent: Запускается на целевых серверах (нодах). Каждые пару секунд читает /proc ОС и делает POST-запрос на главный сервер.

    sysmon-server: Принимает данные, валидирует их и пишет в БД. Параллельно горутина Watchdog проверяет время последней активности каждого агента.

    PostgreSQL: Хранит метрики с автоматической простановкой created_at.

    Grafana: Подключается к БД и рисует графики потребления ресурсов.
...
📈 Настройка Grafana
    ...
    Перейдите по адресу http://IP_ГЛАВНОГО_СЕРВЕРА:3030 (логин/пароль по умолчанию: admin / admin).
    ...
    Создайте дашборд и используйте SQL-запросы для вывода графиков:
